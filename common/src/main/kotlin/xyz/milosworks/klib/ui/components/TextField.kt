		...
		renderer = object : Renderer {
			override fun render(
				...
				guiGraphics.ninePatchTexture(
					...
					node.width,
					node.height,
					if (focused) texture.withSuffix(TextureStates.FOCUSED) else texture.withSuffix(TextureStates.DEFAULT),
				)
...
//				println("highlight: $highlightPos, displayPos: $displayPos, displayedText: ${displayedText.length}")
		...
		modifier = Modifier.onKeyEvent { node, event ->
			...
			println("isdown: ${Minecraft.getInstance()?.options?.keyInventory?.isDown}")
