    ...
    Column(modifier = modifier, verticalArrangement = Arrangement.spacedBy(4)) {
        ...
        if (expanded) {
            Row(horizontalArrangement = Arrangement.spacedBy(5)) {
                ...
                Box(modifier = Modifier.inset(left = 5)) {
                    content()
                }
            }
        }
    ...
    Layout(
        ...
        renderer = object : Renderer {
            override fun render(
                ...
                // Translate to the center of the component for rotation
                guiGraphics.pose().translate(x + node.width / 2f, y + node.height / 2f, 0f)
                guiGraphics.pose().mulPose(Axis.ZP.rotationDegrees(rotation))
                // Translate back to the original top-left corner before drawing
                guiGraphics.pose().translate(-(x + node.width / 2f), -(y + node.height / 2f), 0f)

                guiGraphics.drawString(
                    Minecraft.getInstance().font,
                    ">",
                    x + 1, // small offset to center the character
                    y,
                    KColor.WHITE.argb
                )
